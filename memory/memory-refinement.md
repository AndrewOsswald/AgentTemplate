...
1. **Read `agent/intro.md` as if it was just injected.** Does it clearly bootstrap you into the memory system?
...
Adapt `agent/documentation-refinement.md` for memory. The same structure works:
...
2. **Evaluator phase** — sub-agents read only `agent/intro.md` and navigate from there. They answer questions using only what they find in files. No session history.
