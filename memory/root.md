...
**The agent is an active participant in its own context engineering.** The agent doesn't just consume context — it creates, curates, and refines it. It encodes knowledge into memory, updates documentation, runs refinement audits, and shapes its own behavior in future sessions. This is the core design principle: the agent improves its own working environment over time.
...
- `memory/memory-and-artifacts.md` — why memory and artifacts are separate layers, the two-layer model, how IDE bootstrap files connect to memory
...
- `memory/journal.md` — journal practice: 2-3 entries per session, primarily for the human's benefit
